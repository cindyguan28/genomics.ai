let {margin} = require("@mui/system");
let {addBarPlot} = require("./barPlot");
...
addBarPlotBatch = (barContainerBatch, data) => {
    const svg = d3.select(barContainerBatch).append("svg");
    margin = 10;
    width = 250 - margin;
    height = 400 - margin;
    ...
    this.svg.append("text")
        ...
        .attr("y", 50)
        .attr("font-size", "20px")
        .text("#Gene per Percentile by batch")
    ...
    g.selectAll("rect")
        ...
        .attr("class", "bar")
        .attr("x", function (d) {
            return xScale(d.batch);
        })
        .attr("y", function (d) {

            //TODO: switch between cells Type and Batch
            return yScale(getBatchNum(d.batch));
        })
        .attr("width", xScale.bandwidth())
        .attr("height", function (d) {
            return height - yScale(getBatchNum(d.batch));
        })
    attr("fill", "steelblue")
        .append('title')
        .text(d => getBatchNum(d.batch) / data.length)
